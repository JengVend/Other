...
# 只检查和维护 2 个 UDP 端口给 Hy2 和 TUIC
if [[ $udp_ports -ne 2 ]]; then
    red "端口规则不符合要求，正在调整..."
...
changge_ports() {
reading "将删除全部端口然后随机开放2个udp端口,确定继续吗?(直接回车即确认更换)【y/n】: " choice
...
echo -e "${yellow}本脚本二协议共存${purple}(hysteria2,tuic)${re}"
...
quick_command() {
  ...
  echo "bash <(curl -Ls https://github.com/JengVend/Other/releases/download/sing-box/sb_00_2h1.sh)" >> "$SCRIPT_PATH"
  chmod +x "$SCRIPT_PATH"
